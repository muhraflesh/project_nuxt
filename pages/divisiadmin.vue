<template>
    <section class="abu">
        <div class="columns container abu" style="padding-top:40px">
            <div class="column is-centered is-3 has-background-light kiri atas">
                   <header class="card-header has-background-primary">
                    <p class="card-header-title ">
                    FRONTEND
                    </p>
                </header>
                    <div class="card-content has-background-light has-text-left">
                        <p><b>Ketua</b><br>Muhammad Rafli S</p><br>
                         <p><b>Anggota</b></p>
                        <p>Dewi Ambarwati <br> Salma Faiqah A</p>
                    </div>

            
            
            </div>
            <div class="column is-centered is-9 abu kanan kiri2">
                <div class="row abu">
                    <div>
                    <table>
                        <tr><td><b>TASK</b></td></tr>
                        <tr>
                            <td style="width:10%"><b>Today</b></td>
                            <td style="width:15%"><b>20/9/2019</b></td>
                            <td style="width:10%"><b>10.23 am</b></td>
                            <td style="width:53.5%"></td>
                            <td style="width:6.5%">
                                <nuxt-link class="btn btn-default add" :to="{path: '/add-product'}">
                                    <span class="fa fa-plus-square add"></span>
                                    <b>ADD</b>
                                </nuxt-link>
                            </td>
                        </tr>
                    </table>
                    
                    <div class="card">
                        <div class="card-content">
                            <td>
                            <p class="has-text-left">
                            Membangun home, halaman login, forgot password, profil
                        </p>
                            </td>
                            <td style="width:40%">
                            <span class="icon is-right" style="float:right;">
                                <i class="fa fa-exclamation-circle merah" aria-hidden="true">
                                </i>
                            </span>
                            </td>
                        
                        </div>
                    </div>
                </div>
                </div>
                <div class="row abu">
                    <div class="">
                   <td><b>What's Next ?</b></td>
                <div class="card">
                    <div class="card-content">
                    <td style="width:50%">
                        <span class="icon" style="color:#40e0d0">
                                <i class="fa fa-edit" aria-hidden="true">
                                </i>
                        </span>
                        Mengautentikasi halaman yang telah dibuat
                    </td>
                    <td style="width:10%"><time datetime="2016-1-1">22/07/2019</time></td>
                    <td style="width:10%"><time datetime="2016-1-1">11:09 PM</time></td>
                    <td>
                    </td>
                    </div>
                    <div class="card-content">
                    <td style="width:50%">
                        <span class="icon" style="color:#40e0d0">
                                <i class="fa fa-edit" aria-hidden="true">
                                </i>
                        </span>
                        Membuat halaman web responsive</td>
                    <td style="width:10%"><time datetime="2016-1-1">22/07/2019</time></td>
                    <td style="width:10%"><time datetime="2016-1-1">11:09 PM</time></td>
                    </div>
                    <div class="card-content">
                    <td style="width:50%">
                        <span class="icon" style="color:#40e0d0">
                                <i class="fa fa-edit" aria-hidden="true">
                                </i>
                        </span>
                        Menginstall nuxt js</td>
                    <td style="width:10%"><time datetime="2016-1-1">22/07/2019</time></td>
                    <td style="width:10%"><time datetime="2016-1-1">11:09 PM</time></td>
                    </div>
                    <div class="card-content">
                    <td style="width:50%">
                        <span class="icon" style="color:#40e0d0">
                                <i class="fa fa-edit" aria-hidden="true">
                                </i>
                        </span>
                        Menyempurnakan tampilan web</td>
                    <td style="width:10%"><time datetime="2016-1-1">22/07/2019</time></td>
                    <td style="width:10%"><time datetime="2016-1-1">11:09 PM</time></td>
                    </div>
                    <div class="card-content">
                    <td style="width:50%">
                        <span class="icon" style="color:#40e0d0">
                                <i class="fa fa-edit" aria-hidden="true">
                                </i>
                        </span>
                        Menerapkan git secara keseluruhan</td>
                    <td style="width:10%"><time datetime="2016-1-1">22/07/2019</time></td>
                    <td style="width:10%"><time datetime="2016-1-1">11:09 PM</time></td>
                    </div>
                </div>
            
                </div>
                </div>
            </div>
        </div>
    </section>
</template>

<style>
.kiri{
    display: flex;
    flex-direction: column;
    align-items: stretch;
    height: 650px;
    padding-left: 50px;

}
.kanan{
    display: flex;
    flex-direction: column;
    align-items: stretch;
    height: 650px;
    width: 100px;

}
.abu{
    background-color: #e8e8e8;
}
.merah {
    color: red;
}
.atas {
        width: 200px;
}
.kiri2{
    padding-left: 50px;
}
.add {
    color: #009933
}
</style>